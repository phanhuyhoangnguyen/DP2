...
if (!$connection)
{
    ...
    if (!($_SESSION["role"] == "manager")) {
        echo "<p>Only manager and administrator can create new user.</p>";
    } else {
        $errMsg = "";

        $full_name = mysqli_real_escape_string($connection, $_POST["full_name"]);
        $username = strtolower(mysqli_real_escape_string($connection, $_POST["reg_id"]));
        $password = mysqli_real_escape_string($connection, $_POST["reg_pass"]);
        ...
        if ($confirm_password == "") {
            $errMsg .= "<p>You must confirm your passcode.</p>";
